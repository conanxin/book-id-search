...
每个相邻年度过渡计算固定分值，按白名单累计后判定是否标记为「显著统计差异」。分值与阈值在 `apps/web/src/weread/wereadReadingEvolutionTimeline.ts` 中以常量形式公开：

| 依据（白名单） | 分值 |
|---|---:|
| 年份存在中断（`year_gap`） | 100 |
| 阅读记录数量差异较大（`records_shift`） | 35 |
| 活跃月份数量差异较大（`active_months_shift`） | 25 |
| 年度书目数量差异较大（`matched_books_shift`） | 20 |
| 相邻年度 Top N 榜单重合较低（`low_top_list_overlap`） | 25 |

判定规则：

- **年份中断始终标记为显著统计差异**，无论其他原因总分高低——这反映了相邻年份之间存在缺失自然年的事实性差异。
- 其他四个原因的分值合计达到 **50** 时，标记为「显著统计差异」。
- 分值仅用于确定性展示，**不代表差异的重要性、好坏或原因**，不涉及任何推断性阅读评判或个人化解读。
- 判定仅基于公开数值（记录数、活跃月份数、年度书目数、Top N 重合比例、是否存在年份中断），**不依据书名、作者、主题或任何用户个人化特征**。
- 这些阈值为**确定性启发式规则**，不提供自定义入口。
