...
- Backend scoped **284/284 PASS (18 files)**; Web/search affected **287/287 PASS (12 files)**, including research **81/81 PASS (5 files)**; API/Web builds **PASS**. Existing >500KB Web bundle warning remains.
...
- Whole-branch review: all three accepted findings fixed via RED→GREEN in tested_commit: collapse retains unsaved/stale drafts; concurrent first-create conflict can reload and edit without losing the draft; selected history shows saved time. Added six regression cases. Full browser acceptance ran on `1a7aa59`; final UI fixes rechecked in the real browser (collapse/no extra request/cancel/immutable R1/time/390px). SQL rows unchanged after fixes.
