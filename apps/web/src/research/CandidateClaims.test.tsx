...
import {createCandidateClaim,listCandidateClaims,ProjectApiError} from './api';
import {clearPendingCandidateClaimReceipt,loadPendingCandidateClaimReceipt} from './candidate-claim-draft';
vi.mock('./api',async load=>({...await load<typeof import('./api')>(),createCandidateClaim:vi.fn(),listCandidateClaims:vi.fn()}));
...
beforeEach(()=>{vi.resetAllMocks();clearPendingCandidateClaimReceipt();vi.mocked(listCandidateClaims).mockResolvedValue({claims:[]});vi.mocked(createCandidateClaim).mockResolvedValue({claim});});
afterEach(cleanup);
