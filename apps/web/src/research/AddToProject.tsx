...
import { addCatalogBookToProject, listProjects, ProjectApiError, type Project, type ProjectResearchItem } from "./api";
import { researchEnabled } from "./ProjectsPage";

type Props = { bookId: string; bookTitle: string; onAdded?: (item: ProjectResearchItem) => void };
...
function ProjectSelector({ bookId, bookTitle, onAdded, token }: Props & { token: string | null }) {
  ...
  async function show() {
    ...
    setOpen(true); setError(""); setProjects([]);
    if (!token) return;
    ...
    try {
      ...
      if (!request.signal.aborted) setProjects(data.projects.filter(p => p.lifecycleState === "ACTIVE"));
    } catch (err) {
  ...
  async function add(project: Project) {
    ...
    try {
      ...
      if (!request.signal.aborted) { setPending(false); setAdded(project.name); close(); onAdded?.(result.item); }
    } catch (err) {
  ...
  return <div className="research-add">
    ...
    {added ? <span role="status" className="research-add-success">已加入「{added}」</span> : null}
    {open ? <section className="research-add-panel" aria-label={`将《${bookTitle}》加入研究项目`}>
      {!token ? <p>先进入<Link to="/research/projects">我的研究项目</Link>设置访问凭据。</p> : <>
        ...
        {!loading && !error && projects.length === 0 ? <p>暂无研究项目 <Link to="/research/projects">先创建项目</Link></p> : null}
