...
function fake(options: { project?: object | null; edition?: boolean; duplicate?: boolean; error?: unknown; metadata?: unknown; removed?: boolean } = {}) {
...
describe("Postgres project binding store", () => {
  ...
  it.each([true, false])("removes only owned Edition binding, found=%s", async removed => {
    ...
    expect(s.query).toHaveBeenCalledWith(expect.stringMatching(/DELETE FROM core.project_bindings[\s\S]*id = \$1[\s\S]*project_id = \$2[\s\S]*target_type = 'EDITION'/), ["binding", "project"]);
