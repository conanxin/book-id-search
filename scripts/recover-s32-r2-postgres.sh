...
# This tool NEVER continues R2 execution. It re-proves that the already
# created PostgreSQL runtime is exactly the intended R2 side effect and its
# ONLY write is the atomic, no-overwrite R2 terminal receipt.
...
# Read-only host filesystem operations need privilege on production because
# PGDATA is intentionally mode 0700 and owned by the container postgres UID.
# Tests can supply a helper that emulates privileged read-only probes.
host_read() {
  if [ -n "${S32_RECOVERY_HOST_HELPER:-}" ]; then
    "${S32_RECOVERY_HOST_HELPER}" "$@"
  else
    sudo -n "$@"
  fi
}
...
# --- §4 incident identity / one-shot preconditions -------------------------
...
# --- §5 canonical receipts / manifest / release-scoped secret contract -----
...
[ "$(get_kv "$P/s32-r1.env" S32_RELEASE_FINGERPRINT)" = "$FP" ] \
  && [ "$(get_kv "$P/s32-r1.env" RELEASE_SOURCE_SHA)" = "$SRC" ] \
  || block R1_RELEASE_IDENTITY_MISMATCH
...
DB_NAME="$(get_kv "$PGENV" S32_POSTGRES_DB || true)"
DB_ADMIN="$(get_kv "$PGENV" S32_POSTGRES_USER || true)"
DB_PASSWORD="$(get_kv "$PGENV" S32_POSTGRES_PASSWORD || true)"
APP_PASSWORD="$(get_kv "$PGENV" S32_APP_PASSWORD || true)"
[ "$DB_NAME" = book_id_search_s32 ] && [ "$DB_ADMIN" = s32_admin ] \
  && [ -n "$DB_PASSWORD" ] && [ -n "$APP_PASSWORD" ] || block POSTGRES_ENV_CONTRACT_INVALID
...
MAN_FIELDS="$(python3 - "$P/s32-release-manifest.json" <<'PY'
import json,sys
m=json.load(open(sys.argv[1],encoding='utf-8'))
print(m['pgImageRef'])
print(m['pgImageId'])
PY
)" || block MANIFEST_INVALID
PG_IMAGE_REF="$(printf '%s\n' "$MAN_FIELDS" | sed -n '1p')"
PG_IMAGE_ID="$(printf '%s\n' "$MAN_FIELDS" | sed -n '2p')"
[ -n "$PG_IMAGE_REF" ] && [ -n "$PG_IMAGE_ID" ] || block MANIFEST_INVALID
...
[ "$(get_kv "$CLAIM" STAGE_GROUP)" = R2_R3 ] \
  ...
  && [ "$(get_kv "$CLAIM" EXPLICIT_APPROVAL)" = true ] \
  && [ "$(get_kv "$CLAIM" CONSUMABLE_ONCE)" = true ] \
  || block R2_R3_CLAIM_MISMATCH

# --- §6 exact image: same backend-compatible contract as normal R2 ---------
...
REPO_DIGEST_PROOF="$($DK image inspect "$PG_IMAGE_REF" --format '{{join .RepoDigests " "}}' 2>/dev/null)" || block PG_IMAGE_REPODIGEST_MISMATCH
DIGEST_SEEN=false
...
# --- §7 existing PGDATA: privileged read-only proof, zero mutation ----------
[ -d "$PGDATA" ] && [ ! -L "$PGDATA" ] || block PGDATA_INVALID
[ "$(host_read stat -c '%a' "$PGDATA")" = 700 ] || block PGDATA_MODE_INVALID
[ -n "$(host_read find "$PGDATA" -mindepth 1 -maxdepth 1 -print -quit)" ] || block PGDATA_EMPTY

# --- §8 exact running container + owner + bind mount ------------------------
...
PG_UID="$($DK exec "$CID" id -u postgres)" || block POSTGRES_UID_LOOKUP_FAILED
PG_GID="$($DK exec "$CID" id -g postgres)" || block POSTGRES_GID_LOOKUP_FAILED
printf '%s' "$PG_UID" | grep -qE '^[0-9]+$' || block POSTGRES_UID_LOOKUP_FAILED
printf '%s' "$PG_GID" | grep -qE '^[0-9]+$' || block POSTGRES_GID_LOOKUP_FAILED
[ "$(host_read stat -c '%u:%g' "$PGDATA")" = "${PG_UID}:${PG_GID}" ] || block PGDATA_OWNER_MISMATCH

MOUNTS_JSON="$($DK inspect "$CID" --format '{{json .Mounts}}')" || block POSTGRES_MOUNT_INSPECT_FAILED
python3 - "$PGDATA" "$MOUNTS_JSON" <<'PY' || block POSTGRES_PGDATA_BIND_MISMATCH
import json,sys
source=sys.argv[1]
try:
    ...
    raise SystemExit(1)
matches=[m for m in mounts if m.get('Type')=='bind'
...
DB_SSID="$($DK exec "$CID" psql -U "$DB_ADMIN" -d "$DB_NAME" -At -c \
...
ROLE_COUNT="$($DK exec "$CID" psql -U "$DB_ADMIN" -d "$DB_NAME" -At -c \
