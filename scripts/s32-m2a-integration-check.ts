...
try {
  const name = `s32-m2a-test-${process.pid}-${Date.now()}`;
  ...
  for (let i = 0; i < 60; i += 1) {
    try {
      docker(["exec", container, "pg_isready", "-h", "127.0.0.1", "-U", "s32test", "-d", "s32_m2a_test"]);
      ready = true;
      break;
    } catch {
  ...
  docker(
    ["exec", "-i", container, "psql", "-X", "-U", "s32test", "-d", "s32_m2a_test", "-v", "ON_ERROR_STOP=1", "-f", "-"],
  ...
  const result = spawnSync(
    ...
    ["run", "--maxWorkers=1", "apps/api/src/s32/postgres/research-issue-store.integration.test.ts"],
    {
      cwd: root,
      stdio: "inherit",
      timeout: 300_000,
      env: {
        ...process.env,
        S32_M2A_TEST_DATABASE_URL: `postgresql://s32test:${password}@127.0.0.1:${port}/s32_m2a_test`,
      },
    },
  );
  if (result.status !== 0) throw new Error(`Integration tests failed: ${result.status}`);
  console.log("S32_M2A_REAL_PG=PASS");
} catch (error) {
  ...
  if (container) {
    try {
      const owner = docker([
        "inspect", "--format", '{{ index .Config.Labels "book-id-search.s32-m2a-run" }}', container,
      ]);
