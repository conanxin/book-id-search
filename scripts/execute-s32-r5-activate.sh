...
R4_API_HOST_ID="$(get "$R4" API_IMAGE_ID || true)"; R4_API_CONFIG_DIGEST="$(get "$R4" API_CONFIG_DIGEST || true)"
[ -n "$R4_API_HOST_ID" ] && [ "$R4_API_CONFIG_DIGEST" = "$API_ID" ] || block R4_API_IDENTITY_RECEIPT_INVALID
...
API_HOST_ID="$R4_API_HOST_ID"; API_IDENTITY_MODE="$(get "$R4" API_IDENTITY_MODE || true)"
if [ "${S32_R4_R5_TEST_MODE:-false}" = true ]; then
  ...
  API_HOST_ID="${S32_R4_R5_FAKE_API_HOST_ID:-$R4_API_HOST_ID}"
  [ "$API_HOST_ID" = "$R4_API_HOST_ID" ] || block API_RUNTIME_BINDING_MISMATCH
else
...
python3 - "$POST" "$API_HOST_ID" "$SRC" "$R0" <<'PY' || block R5_POSTVERIFY_FAILED
import json,sys
