...
try {
  const name = `s32-m1c-test-${process.pid}-${Date.now()}`;
  const password = randomBytes(18).toString("hex");
  // Keep the unique name before docker run: a timed-out client may still create a container.
  container = name;
  docker(["run", "--detach", "--rm", "--name", name,
    "--label", `book-id-search.s32-m1c-run=${name}`, "--memory=512m",
    "--tmpfs", "/var/lib/postgresql/data:rw,size=268435456", "-p", "127.0.0.1::5432",
    "-e", "POSTGRES_USER=s32test", "-e", `POSTGRES_PASSWORD=${password}`,
    "-e", "POSTGRES_DB=s32_m1c_test", "postgres:16-alpine"]);
  let ready = false;
  for (let i = 0; i < 60; i++) {
    try { docker(["exec", container, "pg_isready", "-h", "127.0.0.1", "-U", "s32test", "-d", "s32_m1c_test"]); ready = true; break; }
  ...
  const result = spawnSync(resolve(root, "node_modules/.bin/vitest"), ["run", "--maxWorkers=1", "apps/api/src/s32/postgres/project-binding-store.integration.test.ts"], {
    ...
    env: { ...process.env, S32_M1C_TEST_DATABASE_URL: `postgresql://s32test:${password}@127.0.0.1:${port}/s32_m1c_test` },
  });
  if (result.status !== 0) throw new Error(`Integration tests failed: ${result.status}`);
  console.log("S32_M1C_REAL_PG=PASS");
} catch (error) {
  ...
  if (container) {
    try {
      const owner = docker(["inspect", "--format", '{{ index .Config.Labels "book-id-search.s32-m1c-run" }}', container]);
