...
class Env:
    ...
    def run(self, **extra):
        return subprocess.run(
            ["bash", str(EXEC), "--execute-r7", self.fp, SRC, CTRL],
            text=True,
            capture_output=True,
            env=self.env(**extra),
        )
...
class R7ExecutorTests(unittest.TestCase):
    def test_success_requires_claim_and_writes_terminal_receipt(self):
        ...
        self.assertNotIn("TOKEN_SENTINEL", r.stdout + r.stderr + text)
    ...
    def test_failed_acceptance_leaves_incomplete_and_never_auto_retries(self):
        x = Env(); self.addCleanup(x.close)
        r = x.run(S32_R7_FAKE_ACCEPTANCE_EXIT="1")
        self.assertNotEqual(r.returncode, 0)
