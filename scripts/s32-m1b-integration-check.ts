...
try {
  const name = `s32-m1b-test-${process.pid}-${Date.now()}`;
  const password = randomBytes(18).toString("hex");
  // Keep the unique name before docker run: a timed-out client may still create a container.
  container = name;
  docker(["run", "--detach", "--rm", "--name", name,
    "--label", `book-id-search.s32-m1b-run=${name}`, "--memory=512m",
    "--tmpfs", "/var/lib/postgresql/data:rw,size=268435456", "-p", "127.0.0.1::5432",
    "-e", "POSTGRES_USER=s32test", "-e", `POSTGRES_PASSWORD=${password}`,
    "-e", "POSTGRES_DB=s32_m1b_test", "postgres:16-alpine"]);
  let ready = false;
  for (let i = 0; i < 60; i++) {
    try { docker(["exec", container, "pg_isready", "-U", "s32test", "-d", "s32_m1b_test"]); ready = true; break; }
    catch { await new Promise((r) => setTimeout(r, 500)); }
  }
  if (!ready) throw new Error("Disposable PostgreSQL did not become ready");
  docker(["exec", "-i", container, "psql", "-X", "-U", "s32test", "-d", "s32_m1b_test", "-v", "ON_ERROR_STOP=1", "-f", "-"],
  ...
  const result = spawnSync(resolve(root, "node_modules/.bin/vitest"), ["run", "--maxWorkers=1", "apps/api/src/s32/postgres/project-store.integration.test.ts"], {
    cwd: root, stdio: "inherit", timeout: 300_000,
    env: { ...process.env, S32_M1B_TEST_DATABASE_URL: `postgresql://s32test:${password}@127.0.0.1:${port}/s32_m1b_test` },
  });
  if (result.status !== 0) throw new Error(`Integration tests failed: ${result.status}`);
  console.log("S32_M1B_REAL_PG=PASS");
} catch (error) {
  ...
  if (container) {
    try {
      const owner = docker(["inspect", "--format", '{{ index .Config.Labels "book-id-search.s32-m1b-run" }}', container]);
