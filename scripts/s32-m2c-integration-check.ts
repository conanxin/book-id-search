...
try {
  const name = `s32-m2c-test-${process.pid}-${Date.now()}`;
  const password = randomBytes(18).toString("hex");
  container = name;
  docker([
    "run", "--detach", "--rm", "--name", name,
    "--label", `book-id-search.s32-m2c-run=${name}`,
    "--memory=512m",
    "--tmpfs", "/var/lib/postgresql/data:rw,size=268435456",
    "-p", "127.0.0.1::5432",
    "-e", "POSTGRES_USER=s32test",
    "-e", `POSTGRES_PASSWORD=${password}`,
    "-e", "POSTGRES_DB=s32_m2c_test",
    "postgres:16-alpine",
  ]);
  ...
  for (let i = 0; i < 60; i += 1) {
    try {
      docker(["exec", container, "pg_isready", "-h", "127.0.0.1", "-U", "s32test", "-d", "s32_m2c_test"]);
      ready = true;
      break;
    } catch {
  ...
  docker(
    ["exec", "-i", container, "psql", "-X", "-U", "s32test", "-d", "s32_m2c_test", "-v", "ON_ERROR_STOP=1", "-f", "-"],
  ...
  const result = spawnSync(
    ...
    ["run", "--maxWorkers=1", "apps/api/src/s32/postgres/evidence-selection-store.integration.test.ts"],
    {
      cwd: root,
      stdio: "inherit",
      timeout: 300_000,
      env: {
        ...process.env,
        S32_M2C_TEST_DATABASE_URL: `postgresql://s32test:${password}@127.0.0.1:${port}/s32_m2c_test`,
      },
    },
  );
  if (result.status !== 0) throw new Error(`Integration tests failed: ${result.status}`);
  console.log("S32_M2C_REAL_PG=PASS");
} catch (error) {
  ...
  if (container) {
    try {
      const owner = docker([
        "inspect", "--format", '{{ index .Config.Labels "book-id-search.s32-m2c-run" }}', container,
      ]);
