 ...
 *
 * Role split — do NOT merge these responsibilities again:
 *   scripts/health-check.ts            = release/postdeploy Web smoke (S27)
 *   scripts/production-health-check.ts = full production monitoring (this file)
 *
 * Usage:
 *   pnpm health:check
 *   node --experimental-strip-types scripts/production-health-check.ts \
 *     --public-url https://books.conanxin.com \
 *     --expected-docs 5115734 --server-ip 118.195.129.137 \
 *     --json logs/health-check/latest.json \
 *     --markdown logs/health-check/latest.md
 */
