...
for k in AUTHORIZATION_VERSION AUTHORIZED_ACTION STAGE_GROUP S32_RELEASE_FINGERPRINT RELEASE_SOURCE_SHA CONTROL_PLANE_SHA EXPLICIT_APPROVAL CONSUMABLE_ONCE CAPACITY_HARD_ONLY_ACCEPTED PRODUCTION_WRITE_EXECUTED; do [ "$(grep -cE "^${k}=" "$AUTH" || true)" = 1 ] || block AUTHORIZATION_INCOMPLETE; done
get(){ grep -E "^$1=" "$AUTH"|head -1|cut -d= -f2-; }
[ "$(get CAPACITY_HARD_ONLY_ACCEPTED)" = true ] || [ "$(get CAPACITY_HARD_ONLY_ACCEPTED)" = false ] || block AUTHORIZATION_PLAN_MISMATCH
