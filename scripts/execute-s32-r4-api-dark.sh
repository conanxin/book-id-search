...
API_HOST_ID="$API_ID"; API_IDENTITY_MODE=CLASSIC_CONFIG
if [ "${S32_R4_R5_TEST_MODE:-false}" = true ]; then
  API_OVERRIDE="$ROOT/api-production.override.yml"; WEB_OVERRIDE="$ROOT/web-production.override.yml"; :>"$API_OVERRIDE"; :>"$WEB_OVERRIDE"
  API_HOST_ID="${S32_R4_R5_FAKE_API_HOST_ID:-$API_ID}"
  [ "$API_HOST_ID" = "$API_ID" ] || API_IDENTITY_MODE=OCI_MANIFEST
else
  API_HOST_ID="$(sudo -n docker image inspect "$API_TAG" --format '{{.Id}}')" || block API_IMAGE_NOT_LOCAL
  REV="$(sudo -n docker image inspect "$API_TAG" --format '{{index .Config.Labels "org.opencontainers.image.revision"}}')" || block API_RELEASE_IDENTITY_MISMATCH
  [ "$REV" = "$SRC" ] || block API_RELEASE_IDENTITY_MISMATCH
  if [ "$API_HOST_ID" != "$API_ID" ]; then
...
VERIFY_ERR="$(mktemp)"
if ! python3 - "$R0" "$POST" "$API_HOST_ID" "$SRC" 2>"$VERIFY_ERR" <<'PY'
import json,sys
r={}
...
if str(p.get('stats',{}).get('numberOfDocuments')) != r.get('MEILI_DOCUMENTS'): raise SystemExit('MEILI_DOCUMENT_COUNT_DRIFT')
...
T="$(mktemp "$ROOT/progress/.r4-result.XXXXXX")"; printf 'STATUS=PASS\nSTAGE=R4\nR4_API_DARK=PASS\nS32_RELEASE_FINGERPRINT=%s\nRELEASE_SOURCE_SHA=%s\nMEILI_DOCUMENTS=%s\nAPI_IMAGE_ID=%s\nAPI_CONFIG_DIGEST=%s\nAPI_IDENTITY_MODE=%s\nAPI_REVISION=%s\nS32_FEATURES_ENABLED=false\n' "$FP" "$SRC" "$BASE_MEILI_DOCUMENTS" "$API_HOST_ID" "$API_ID" "$API_IDENTITY_MODE" "$SRC">"$T"; chmod 600 "$T"; ln -- "$T" "$RESULT"; rm -f "$T"; printf 'STATUS=PASS\nR4_API_DARK=PASS\nS32_FEATURES_ENABLED=false\n'
 || block API_RELEASE_IDENTITY_MISMATCH
    sudo -n docker image save "$API_TAG" | python3 "$SCRIPT_DIR/verify-s32-api-image-archive.py" \
      --expected-manifest-digest "$API_HOST_ID" \
      --expected-config-digest "$API_ID" \
      --expected-revision "$SRC" >/dev/null || block API_IMAGE_ARCHIVE_IDENTITY_MISMATCH
    API_IDENTITY_MODE=OCI_MANIFEST
  fi
fi
umask 077; T="$(mktemp "$ROOT/progress/.r4-start.XXXXXX")"; printf 'STATUS=STARTED\nSTAGE=R4\nS32_RELEASE_FINGERPRINT=%s\n' "$FP">"$T"; chmod 600 "$T"; ln -- "$T" "$START" || { rm -f "$T"; block INCOMPLETE_R4; }; rm -f "$T"
CMD=(docker compose --project-directory "$ROOT" --env-file "$PG_ENV" -f "$ROOT/docker-compose.yml" -f "$ROOT/docker-compose.override.yml" -f "$API_OVERRIDE" -f "$WEB_OVERRIDE" -f "$OVERRIDE" up -d --no-build --no-deps api)
...
rm -f "$VERIFY_ERR"
[ "${S32_R4_R5_TEST_MODE:-false}" = true ] || rm -f "$POST"
T="$(mktemp "$ROOT/progress/.r4-result.XXXXXX")"; printf 'STATUS=PASS\nSTAGE=R4\nR4_API_DARK=PASS\nS32_RELEASE_FINGERPRINT=%s\nRELEASE_SOURCE_SHA=%s\nMEILI_DOCUMENTS=%s\nAPI_IMAGE_ID=%s\nAPI_REVISION=%s\nS32_FEATURES_ENABLED=false\n' "$FP" "$SRC" "$BASE_MEILI_DOCUMENTS" "$API_ID" "$SRC">"$T"; chmod 600 "$T"; ln -- "$T" "$RESULT"; rm -f "$T"; printf 'STATUS=PASS\nR4_API_DARK=PASS\nS32_FEATURES_ENABLED=false\n'
