...
API_OBSERVED_ID=""; API_IDENTITY_MODE=""
if [ "${S32_R4_R5_TEST_MODE:-false}" = true ]; then
  ...
  API_OBSERVED_ID="${S32_R4_R5_FAKE_API_OBSERVED_ID:-$API_ID}"; API_IDENTITY_MODE="${S32_R4_R5_FAKE_API_IDENTITY_MODE:-CONFIG_DIGEST}"
else
...
T="$(mktemp "$ROOT/progress/.r4-result.XXXXXX")"; printf 'STATUS=PASS\nSTAGE=R4\nR4_API_DARK=PASS\nS32_RELEASE_FINGERPRINT=%s\nRELEASE_SOURCE_SHA=%s\nMEILI_DOCUMENTS=%s\nAPI_IMAGE_ID=%s\nAPI_CONFIG_DIGEST=%s\nAPI_IDENTITY_MODE=%s\nAPI_REVISION=%s\nS32_FEATURES_ENABLED=false\n' "$FP" "$SRC" "$BASE_MEILI_DOCUMENTS" "$API_OBSERVED_ID" "$API_ID" "$API_IDENTITY_MODE" "$SRC">"$T"; chmod 600 "$T"; ln -- "$T" "$RESULT"; rm -f "$T"; printf 'STATUS=PASS\nR4_API_DARK=PASS\nS32_FEATURES_ENABLED=false\n'
