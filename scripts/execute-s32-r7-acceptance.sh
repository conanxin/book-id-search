...
validate_common() {
  ...
  printf '%s' "$BASE_MEILI_DOCUMENTS" | grep -qE '^[0-9]+
...
validate_api_acceptance() {
  ...
  [ "$(get_kv "$file" STATUS || true)" = PASS ]     && [ "$(get_kv "$file" PROJECT_NAME || true)" = "$expected_project" ]     && [ "$(get_kv "$file" LEGACY_SEARCH_REGRESSION || true)" = PASS ]     && [ "$(get_kv "$file" ASSESSMENT_REPLAY || true)" = PASS ]     && [ "$(get_kv "$file" S32_BACKEND_ACCEPTANCE || true)" = PASS ]     && [ "$(get_kv "$file" MEILI_DOCUMENTS || true)" = "$BASE_MEILI_DOCUMENTS" ]     && [ "$(get_kv "$file" ACCEPTANCE_PROJECT_RETAINED || true)" = YES ]     || return 1
...
if [ "$MODE" = --execute-r7-api ]; then
  ...
  if [ "${S32_R7_TEST_MODE:-false}" = true ]; then
    ...
    (
      cd "$ROOT"
      env         S32_PRIVATE_API_TOKEN="$TOKEN"         S32_RELEASE_FINGERPRINT="$FP"         S32_EXPECTED_DOCUMENT_COUNT="$BASE_MEILI_DOCUMENTS"         S32_API_BASE_URL="${S32_API_BASE_URL:-https://books.conanxin.com}"         S32_PUBLIC_URL="${S32_PUBLIC_URL:-https://books.conanxin.com}"         pnpm s32:production:acceptance
...
EXPECTED_PROJECT="[S32 Production Acceptance] ${FP:0:12}"
[ "$(get_kv "$API_RESULT" PROJECT_NAME || true)" = "$EXPECTED_PROJECT" ]   && [ "$(get_kv "$WEB_RESULT" STATUS || true)" = PASS ]   && [ "$(get_kv "$WEB_RESULT" STAGE || true)" = R7_WEB ]   && [ "$(get_kv "$WEB_RESULT" S32_RELEASE_FINGERPRINT || true)" = "$FP" ]   && [ "$(get_kv "$WEB_RESULT" PROJECT_ID || true)" = "$PROJECT_ID" ]   && [ "$(get_kv "$WEB_RESULT" S32_WEB_ACCEPTANCE || true)" = PASS ]   && [ "$(get_kv "$WEB_RESULT" MOBILE_390x844 || true)" = PASS ]   && [ "$(get_kv "$WEB_RESULT" NO_HORIZONTAL_OVERFLOW || true)" = PASS ]   && [ "$(get_kv "$WEB_RESULT" RUNNER_VERSION || true)" = 1 ]   && [ "$(get_kv "$WEB_RESULT" RUNNER_SOURCE_SHA || true)" = "$CTRL" ]   || block R7_WEB_ACCEPTANCE_CONTRACT_INVALID

python3 - "$WEB_RESULT" <<'PY' || block R7_WEB_ACCEPTANCE_HASH_INVALID
import hashlib, pathlib, sys
p=pathlib.Path(sys.argv[1])
raw=p.read_text(encoding='utf-8')
lines=raw.splitlines()
hash_lines=[line for line in lines if line.startswith('RECEIPT_SHA256=')]
if len(hash_lines)!=1: raise SystemExit(1)
expected=hash_lines[0].split('=',1)[1]
if len(expected)!=64 or any(c not in '0123456789abcdef' for c in expected): raise SystemExit(1)
base='\n'.join(line for line in lines if not line.startswith('RECEIPT_SHA256='))+'\n'
if hashlib.sha256(base.encode('utf-8')).hexdigest()!=expected: raise SystemExit(1)
PY
...
printf 'STATUS=PASS\nR7_ACCEPTANCE=PASS\nS32_WEB_ACCEPTANCE=PASS\nMOBILE_390x844=PASS\nS32_RELEASE_FINGERPRINT=%s\nPROJECT_ID=%s\nACCEPTANCE_PROJECT_RETAINED=YES\nAUTO_RETRY=NO\n'   "$FP" "$PROJECT_ID"
 || block R0_MEILI_DOCUMENTS_INVALID
  [ "$(get_kv "$R6" STATUS || true)" = PASS ]     && [ "$(get_kv "$R6" R6_WEB || true)" = PASS ]     && [ "$(get_kv "$R6" S32_RELEASE_FINGERPRINT || true)" = "$FP" ]     && [ "$(get_kv "$R6" RELEASE_SOURCE_SHA || true)" = "$SRC" ]     && [ "$(get_kv "$R6" MEILI_DOCUMENTS || true)" = "$BASE_MEILI_DOCUMENTS" ]     || block R6_NOT_PASS
