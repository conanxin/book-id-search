...
class Env:
 def __init__(self):
  ...
  self.api_observed='sha256:'+'a'*64; self.pg_observed='sha256:'+'b'*64; self.web_observed='sha256:'+'c'*64
  self.r2=self.root/'progress'/f's32-rollout-{self.fp}-R2.result.env'; self.r2.write_text(f'STATUS=PASS\nR2_POSTGRES=PASS\nS32_RELEASE_FINGERPRINT={self.fp}\nPG_IMAGE_ID={self.pg_observed}\n'); os.chmod(self.r2,0o600)
  self.r5=self.root/'progress'/f's32-rollout-{self.fp}-R5.result.env'; self.r5.write_text(f'STATUS=PASS\nR5_S32_ACTIVATION=PASS\nS32_RELEASE_FINGERPRINT={self.fp}\nRELEASE_SOURCE_SHA={SRC}\nAPI_IMAGE_ID={self.api_observed}\nAPI_CONFIG_DIGEST=sha256:'+('2'*64)+'\n'); os.chmod(self.r5,0o600)
  ...
  self.pre=self.root/'pre.json'; self.pre.write_text(json.dumps({'services':{'web':{'cid':'oldw','startedAt':'oldt','imageId':'oldwi','revision':'d'*40},'api':{'cid':'api','startedAt':'at','imageId':self.api_observed,'revision':SRC},'meilisearch':{'cid':'m','startedAt':'mt','imageId':'mi'},'postgres':{'cid':'p','startedAt':'pt','imageId':self.pg_observed}},'httpStatus':200}))
  self.post=self.root/'post.json'; self.post.write_text(json.dumps({'services':{'web':{'cid':'neww','startedAt':'newt','imageId':self.web_observed,'revision':SRC},'api':{'cid':'api','startedAt':'at','imageId':self.api_observed,'revision':SRC},'meilisearch':{'cid':'m','startedAt':'mt','imageId':'mi'},'postgres':{'cid':'p','startedAt':'pt','imageId':self.pg_observed}},'httpStatus':200,'stats':{'numberOfDocuments':5115734,'isIndexing':False},'searches':{k:{'status':'PASS'} for k in ['ISBN','SSID','DXID','title','author','publisher']}}))
  (self.root/'static/index.js').write_text('safe bundle'); self.log=self.root/'cmd.log'
 def close(self): self.t.cleanup()
 def env(self):
  e=os.environ.copy(); e.update(BOOK_ID_SEARCH_REPO_ROOT=str(self.root),S32_R0_RECEIPT=str(self.r0),S32_R2_RECEIPT=str(self.r2),S32_R5_RECEIPT=str(self.r5),S32_R6_CAPACITY_RECEIPT=str(self.cap),S32_RELEASE_MANIFEST_JSON=str(self.man),S32_R6_CANDIDATE_JSON=str(self.candidate),S32_API_ENV_FILE=str(self.api),S32_R6_STATIC_DIR=str(self.root/'static'),S32_R6_TEST_MODE='true',S32_R6_PRE_FACTS_JSON=str(self.pre),S32_R6_POST_FACTS_JSON=str(self.post),S32_R6_COMMAND_LOG=str(self.log),S32_R6_FAKE_WEB_OBSERVED_ID=self.web_observed,S32_R6_FAKE_WEB_IDENTITY_MODE='MANIFEST_DIGEST'); return e
...
class T(unittest.TestCase):
 def test_success_switches_web_only_and_records_parity(self):
  x=Env(); self.addCleanup(x.close); r=x.run(); self.assertEqual(r.returncode,0,r.stdout+r.stderr); self.assertIn('R6_WEB=PASS',r.stdout); self.assertIn('--no-build --no-deps web',x.log.read_text()); receipt=next(x.root.glob('progress/*R6.result.env')).read_text(); self.assertIn(f'WEB_IMAGE_ID={x.web_observed}',receipt); self.assertIn('WEB_CONFIG_DIGEST=sha256:'+'4'*64,receipt); self.assertIn('WEB_IDENTITY_MODE=MANIFEST_DIGEST',receipt)
 ...
 def test_api_host_identity_must_match_r5_receipt(self):
  x=Env(); self.addCleanup(x.close); f=json.loads(x.pre.read_text()); f['services']['api']['imageId']='sha256:'+'2'*64; x.pre.write_text(json.dumps(f)); r=x.run(); self.assertNotEqual(r.returncode,0); self.assertIn('API_RELEASE_PARITY_REQUIRED',r.stdout+r.stderr)
