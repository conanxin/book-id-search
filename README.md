...
## S14 Import Benchmark And Full Import Plan

当前本地 `books` index 保留 500000 条真实记录，可作为演示索引继续使用。S14 benchmark 使用独立临时索引，没有重置或删除 `books`。
...
`--search-raw-info false` 只是不把原始整行 `rawInfo` 放进全文搜索字段；`rawInfo` 仍然会保存在文档里，并在前端详情中可查看/复制。SSID、DXID、ISBN、书名、作者、出版社仍参与搜索。完整策略见 [reports/FULL_IMPORT_STRATEGY.md](reports/FULL_IMPORT_STRATEGY.md)。

云端全量导入建议在 `tmux` 或 `screen` 里运行。真实 TXT、`meili_data`、checkpoint JSON、`.env`、`meilisearch.exe` 不进入 Git。
