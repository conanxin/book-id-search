...
\echo '=== ASSERT: exact 22 core table names ==='
DO $$ DECLARE names text; expected text; BEGIN
    SELECT string_agg(table_name, ',' ORDER BY table_name) INTO names
    FROM information_schema.tables WHERE table_schema = 'core';
    expected := 'actors,assessments,claim_relations,claims,editions,evidence_manifest_items,evidence_manifests,external_identities,issue_resolutions,note_revisions,note_revision_parents,notes,project_bindings,projects,research_issue_claims,research_issues,research_runs,source_asset_parents,source_assets,sources,works';
    IF names <> expected THEN
        RAISE EXCEPTION 'S32_ASSERT_FAIL: core table names mismatch (got: %)', names;
    END IF;
END $$;

\echo '=== ASSERT: exact 4 ops table names ==='
DO $$ DECLARE names text; expected text; BEGIN
    SELECT string_agg(table_name, ',' ORDER BY table_name) INTO names
    FROM information_schema.tables WHERE table_schema = 'ops';
    expected := 'idempotency_keys,integrity_check_runs,outbox_events,projection_generations';
    IF names <> expected THEN
        RAISE EXCEPTION 'S32_ASSERT_FAIL: ops table names mismatch (got: %)', names;
    END IF;
END $$;
...
DO $$ DECLARE conname text; deferrable bool; deferred bool; BEGIN
    SELECT c.conname, c.condeferrable, c.condeferred INTO conname, deferrable, deferred
    ...
    IF NOT deferrable THEN RAISE EXCEPTION 'S32_ASSERT_FAIL: notes FK not DEFERRABLE'; END IF;
    IF NOT deferred THEN RAISE EXCEPTION 'S32_ASSERT_FAIL: notes FK not INITIALLY DEFERRED'; END IF;
END $$;
...
DO $$ DECLARE conname text; deferrable bool; deferred bool; BEGIN
    ...
    IF NOT deferrable THEN RAISE EXCEPTION 'S32_ASSERT_FAIL: research_issues FK not DEFERRABLE'; END IF;
    IF NOT deferred THEN RAISE EXCEPTION 'S32_ASSERT_FAIL: research_issues FK not INITIALLY DEFERRED'; END IF;
END $$;
